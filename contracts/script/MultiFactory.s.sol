...
contract DeployMultiFactory is Script, BaseDeployer {
    ...
    function createDeployMultichain(
        ...
        console2.log("Factory create2 address:", create2addrCounter, "\n");

        for (uint256 i; i < deployForks.length; ) {
            console2.log("Deploying Factory to chain: ", uint(deployForks[i]), "\n");
    ...
    function chainDeployCounter() private broadcast(deployerPrivateKey) {
        ...
        Factory factory = new Factory{salt: counterSalt}(entryPoint);

        require(create2addrCounter == address(factory), "Address mismatch Factory ");
